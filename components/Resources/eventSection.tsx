...
const EventsSection: FC = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const res = await makeApiRequest('/gallery', 'get');
                setEvents(res.data);
            } catch (err: any) {
    ...
    if (loading) {
        return (
            <div className="px-6 py-10 text-center text-lg text-gray-600">
    ...
    if (error) {
        return (
            <div className="px-6 py-10 text-center text-lg text-red-600">
                {error}
            </div>
        );
    }
